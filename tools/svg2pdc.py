...
def convert_to_pebble_coordinates(point, precise=False):
    ...
    # Accept any coordinate: snap silently to the nearest supported point.
    valid = True
    point = nearest
...
def parse_path(element, translate, stroke_width, stroke_color, fill_color, precise, raise_error):
    ...
    if d is not None:
        ...
        # Drop degenerate paths (e.g. a lone moveto): a 0/1-point command is
        # invalid in PDC and can make the whole image fail to load.
        if len(points) < 2:
            return None
